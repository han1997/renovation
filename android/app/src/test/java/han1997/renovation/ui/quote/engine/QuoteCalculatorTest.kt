...
class QuoteCalculatorTest {
    ...
    private val catalog = DecoboxCatalogJson(
        version = "test",
        source = "test",
        craft = han1997.renovation.data.knowledge.CraftParams(15, 20),
        managementFeeRate = 8,
        defaultHouse = han1997.renovation.data.knowledge.DefaultHouse(90.0, 120000.0, 2.4),
        ...
        partialFees = han1997.renovation.data.knowledge.PartialFees(25, 8, 300, 800),
        partialItems = listOf(
            han1997.renovation.data.knowledge.PartialItem(
                id = "wall-refresh",
                label = "墙面刷新",
                unit = "m2",
                integerQty = false,
                needsArea = false,
                tiers = listOf(
                    han1997.renovation.data.knowledge.PartialTier("light", "墙面结实,只是旧了", 45),
                    han1997.renovation.data.knowledge.PartialTier("full", "有掉皮、发霉、开裂", 85),
                ),
            ),
        ),
    )
