...
import han1997.renovation.data.db.CategoryWithSpent
import han1997.renovation.data.db.ExpenseEntity
import han1997.renovation.ui.components.BudgetProgressBar
...
fun BudgetScreen(onOpenQuote: () -> Unit = {}) {
    ...
    han1997.renovation.ui.components.OperationFeedback(vm, snackbar)
            ...
            item {
                SectionCard {
                    Column {
                        ...
                        han1997.renovation.ui.components.MoneyLine("分类计划", totalPlanned)
                        han1997.renovation.ui.components.MoneyLine("实际支出", totalSpent)
                        if (totalOver > 0) {
                            Text("分类超支合计 ¥${han1997.renovation.util.MoneyUtil.formatFull(totalOver)}", color = MaterialTheme.colorScheme.error, style = MaterialTheme.typography.bodySmall, modifier = Modifier.padding(top = 6.dp))
                        }
                    }
                }
            }
                    ...
                    Column {
                        Row(verticalAlignment = Alignment.CenterVertically) {
                            ...
                            Column(modifier = Modifier.weight(1f).padding(start = 8.dp)) {
                                ...
                                Text("${han1997.renovation.util.MoneyUtil.formatFull(cat.spentCents)} / ${han1997.renovation.util.MoneyUtil.formatFull(cat.plannedCents)} 元", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
                            }
                            if (cat.overCents > 0) {
                            ...
                            expenses.forEach { ex ->
                                Row(verticalAlignment = Alignment.CenterVertically, modifier = Modifier.fillMaxWidth().padding(vertical = 2.dp)) {
                                    Column(modifier = Modifier.weight(1f)) {
                                        ...
                                        Text("${DateUtil.formatCN(ex.date)}  ·  ¥${han1997.renovation.util.MoneyUtil.formatFull(ex.amountCents)}", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
                                    }
