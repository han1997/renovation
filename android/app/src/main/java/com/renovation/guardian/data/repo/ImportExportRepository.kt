...
import com.renovation.guardian.data.db.*
...
import com.renovation.guardian.domain.quote.QuotePlanState
import com.renovation.guardian.domain.quote.QuoteValidation
import com.renovation.guardian.domain.planner.PlannerState
...
internal data class BackupV2(
    ...
    val appId: String = "com.renovation.guardian",
...
class ImportExportRepository(private val db: AppDatabase, private val knowledge: KnowledgeCache? = null) {
        ...
        val value = when (version) {
            2 -> {
                ...
                json.decodeFromJsonElement(BackupV2.serializer(), root).also {
                    require(it.appId == "com.renovation.guardian") { "不是装修管家 Android 备份" }
                }
            }
            1 -> legacy(root)
    ...
    private suspend fun validate(p: BackupV2) {
        ...
        p.plannerState?.let {
            ...
            knowledge?.decoboxRequirements?.let { catalog -> com.renovation.guardian.domain.planner.PlannerRules.validateCatalogReferences(state, catalog) }
...
private data class ProfileExport(
    ...
    fun toEntity() = HouseProfileEntity(
        ...
        gradeId = grade,
        startDate = startDate,
        totalBudgetCents = com.renovation.guardian.util.MoneyUtil.fromYuan(totalBudget),
        styleId = styleId,
        styleQuizAt = null,
        createdAt = createdAt,
    )
...
private data class BudgetCategoryExport(
    ...
    fun toEntity() = com.renovation.guardian.data.db.BudgetCategoryEntity(
        ...
        name = name,
        emoji = emoji,
        plannedCents = com.renovation.guardian.util.MoneyUtil.fromYuan(planned),
...
private data class ExpenseExport(
    ...
    fun toEntity() = ExpenseEntity(
        id = id,
        name = name,
        amountCents = com.renovation.guardian.util.MoneyUtil.fromYuan(amount),
        categoryId = catId,
        date = date,
        note = note,
        createdAt = createdAt,
